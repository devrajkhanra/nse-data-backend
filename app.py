...
#                 # Download MA data (commented out as it seems to be failing)
#                 # try:
#                 #     ma_url = f"https://archives.nseindia.com/content/indices/MA{formatted_date_ddmmyy}.csv"
#                 #     ma_path = os.path.join(DATA_DIR, "ma", f"MA{formatted_date_ddmmyy}.csv")
#                 #     file_handler.download_file(ma_url, ma_path)
...
#                 # Download 5min data for each Nifty50 stock using yfinance
#                 successful_5min_downloads = 0
#                 failed_5min_downloads = 0
...
#                     try:
#                         # Use yfinance to download 5min data
#                         ticker = f"{stock}.NS"
#                         start_date = datetime.strptime(date, '%Y-%m-%d')
#                         end_date = start_date + timedelta(days=1)
                        
#                         print(f"Downloading 5min data for {stock} from {start_date} to {end_date}")
...
#                             # Filter for the specific date only (in case we got data from multiple days)
#                             target_date = pd.Timestamp(start_date).date()
#                             df_filtered = df[pd.to_datetime(df.index).date == target_date]
                            
#                             if not df_filtered.empty:
#                                 # Save to CSV
#                                 df_filtered.to_csv(csv_path)
#                                 print(f"Successfully downloaded 5min data for {stock} via yfinance ({len(df_filtered)} records)")
#                                 successful_5min_downloads += 1
#                             else:
#                                 print(f"No trading data for {stock} on {date} (weekend/holiday)")
#                                 failed_5min_downloads += 1
...
def fetch_yf_5min_data(ticker, target_date):
    try:
        ...
        df = yf_ticker.history(interval="5m", start=start_dt, end=end_dt)

        if df.empty or 'Close' not in df.columns:
            return None

        df.index = df.index + pd.Timedelta(hours=5, minutes=30)
        df_filtered = df[pd.to_datetime(df.index).date == start_dt.date()]
        return df_filtered.reset_index() if not df_filtered.empty else None
    except Exception as e:
        print(f"[ERROR] Failed for {ticker} on {target_date}: {e}")
        return None
...
def download_data():
    try:
        ...
        if date_type == "single" and isinstance(dates_input, str):
            dates = [dates_input]
        elif date_type == "range" and isinstance(dates_input, list) and len(dates_input) == 2:
            dates = date_utils.get_date_range(*dates_input)
        else:
        ...
        for idx, date in enumerate(dates):
            ...
            try:
                # Download indices, stock data and ma
                file_handler.download_file(
                ...
                # Download 5min data per stock
                for s_idx, stock in enumerate(nifty50_list):
                    ...
                    ticker = f"{stock}.NS"
                    df_5min = fetch_yf_5min_data(ticker, date)
                    if df_5min is not None:
                        df_5min.to_csv(file_path)

                    if s_idx % 10 == 0:
                        progress["status"] = f"Processed {s_idx + 1}/{len(nifty50_list)} stocks"
                        file_handler.update_progress(progress)
                        time.sleep(0.5)
